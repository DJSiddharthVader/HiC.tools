...
generate_distance_expectation_calculation_cmds <- function(
    ...
    mkdir.cmd <- glue("mkdir -p {output_dir}")
    main.cmd  <- glue("cooltools expected-{contact.type} --smooth --aggregate-smoothed -p {threads} -o {output.filepath} {mcool.uri}")
    # Paste  all commands together in one line to run in bash
    tibble_row(
...
generate_all_distance_expectation_calculation_cmds <- function(
    ...
    mutate(
        cmd.data=
            pmap(
                .l=.,
                .f=generate_distance_expectation_calculation_cmds,
                .progress=TRUE
            )
    ) %>%
    unnest(cmd.data) %>% 
